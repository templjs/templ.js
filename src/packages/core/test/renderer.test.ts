...
import { literal, binary, unary, template } from './renderer/renderer.test-helpers';
    ...
    describe('Class: Renderer', () => {
      ...
      `;
        ...
        describe('Renderer internals and options', () => {
          ...
          it('should not throw for undefined variable even if throwOnError is true', () => {
            ...
            expect(() => {
              ...
              expect(renderResult.errors.map((e: RenderError) => e.type)).toContain(
                'undefined_variable'
              );
            }).not.toThrow();
          });
          ...
          it('should report undefined_variable error but success=true if throwOnError is false', () => {
            ...
            expect(result.success).toBe(true);
            expect(result.errors.some((e: RenderError) => e.type === 'undefined_variable')).toBe(
              true
            );
          });
          ...
          it('should handle unknown node types gracefully', () => {
            ...
            expect(result.success).toBe(false);
            expect(
              result.errors.some((e: RenderError) => e.message.includes('Unknown node type'))
            ).toBe(true);
          });
          ...
          it('should handle invalid AST node', () => {
            ...
            expect(result.success).toBe(false);
            expect(
              result.errors.some((e: RenderError) =>
                e.message.includes('Unknown or invalid AST node')
              )
            ).toBe(true);
          });
        });
        ...
        describe('filters', () => {
          ...
          describe('datetime', () => {
            it('should render ISO date string as output', () => {
              const template = '{{ date }}';
              const tokens = tokenize(template);
              const parseResult = parse(tokens);
              if (!parseResult.ast) throw new Error('Parse failed');
              const result = render(parseResult.ast, { date: '2023-01-01T00:00:00Z' });
              expect(result.output).toBe('2023-01-01T00:00:00Z');
            });

            // Add more datetime function tests here as you extract them
          });
          ...
          describe('number', () => {
            it('should handle numeric values in output', () => {
              ...
              const parseResult = parse(tokens);
              if (!parseResult.ast) throw new Error('Parse failed');
              const result = render(parseResult.ast, { value: 42 });
            ...
            // Add more number function tests here as you extract them
          });
