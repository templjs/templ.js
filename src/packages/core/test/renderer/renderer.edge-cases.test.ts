...
import type { ASTNode, ExpressionNode, TemplateNode } from '../../src/parser/types.js';
import { parse } from '../../src/parser/parser.js';
import { tokenize } from '../../src/lexer/lexer.js';
import { Renderer, render } from '../../src/renderer/renderer.js';
import { literal, variable, binary, filtered, template, POS } from './renderer.test-helpers.js';
...
describe('Renderer', () => {
  describe('Renderer edge cases', () => {
    ...
    describe('Data Access Edge Cases', () => {
      it('uses an empty root context when input data is not an object', () => {
        const parseResult = parse(tokenize('{{ missing }}'));
        const result = render(parseResult.ast!, 42 as unknown as Record<string, unknown>);

        expect(result.output).toBe('');
        expect(result.errors.some((e) => e.type === 'undefined_variable')).toBe(true);
    ...
    describe('error handling paths', () => {
      ...
      it('falls back to default maxDepth when maxDepth is explicitly undefined', () => {
        const loopAst: TemplateNode = {
          type: 'template',
          start: POS,
          end: POS,
        ...
        const renderer = new Renderer({ maxDepth: undefined as unknown as number });
        const result = renderer.render(loopAst, { items: [1, 2] });
      ...
      it('coerces non-Error throw values into runtime_error messages', () => {
        const ast = {
          get type() {
            throw 'string-throw';
          },
      ...
      it('reports invalid expression_statement nodes with unstable type metadata', () => {
        let typeReads = 0;
        const unstableExpressionStatement = {
          get type() {
            typeReads += 1;
            if (typeReads <= 3) {
        ...
        const ast: TemplateNode = {
          ...
          children: [unstableExpressionStatement],
        };

        const result = render(ast, {});

        expect(result.output).toBe('');
        expect(result.errors.some((e) => e.message === 'Invalid or missing expression node')).toBe(
          true
        );
        expect(result.success).toBe(false);
      });
