...
describe('TemplateParser - public extractStatementContent / extractExpressionContent', () => {
  describe('extractStatementContent', () => {
    ...
    // Case 3: no metadata and no explicit delimiters → return trimmed input unchanged
    it('returns trimmed content when neither token metadata nor explicit delimiters are provided', () => {
      const parser = makeParser();
      expect(parser.extractStatementContent('  if user.active  ').content).toBe('if user.active');
    });

    it('returns trimmed token content when metadata is undefined and no explicit delimiters are provided', () => {
      const parser = makeParser();
      const token: ExtractTokenInput = {
        content: '{% if user.active %}',
    ...
    it('returns trimmed content when token metadata provides only delimiterStart', () => {
      const parser = makeParser();
      const token: ExtractTokenInput = {
        content: '{% x %}',
        delimiterStart: '{%',
      ...
      expect(parser.extractStatementContent(token).content).toBe('{% x %}');
    });

    it('returns trimmed content when token metadata provides only delimiterEnd', () => {
      const parser = makeParser();
      const token: ExtractTokenInput = {
        content: '{% x %}',
        delimiterStart: undefined,
  ...
  describe('extractExpressionContent', () => {
    ...
    it('returns trimmed content when neither token metadata nor explicit delimiters are provided', () => {
      const parser = makeParser();
      expect(parser.extractExpressionContent('  user.name  ').content).toBe('user.name');
    });
    ...
    it('returns trimmed content when token metadata provides only delimiterStart', () => {
      ...
      expect(parser.extractExpressionContent(token).content).toBe('{{x}}');
    });
