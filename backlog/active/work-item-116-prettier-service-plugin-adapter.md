...
- [ ] Define Prettier adapter capabilities (supported formats: markdown, json, yaml, html).
- [ ] Implement runtime planning logic for Prettier availability (configuration-based, host LS gating).
- [ ] Audit `createPrettierHostServicePlugin` implementation.
- [ ] Move non-trivial logic (>20 lines) into separate files; keep trivial logic in service-plugins.ts.
- [ ] Add capability-aware gating for Prettier features.
- [ ] Add tests for adapter initialization, capability publishing, and configuration gating.
...
- [ ] Prettier adapter publishes correct capabilities.
- [ ] Runtime planning prevents Prettier from engaging unless explicitly configured.
